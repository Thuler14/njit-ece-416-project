...
void sendAck(uint16_t seq, float setpointF, uint32_t ms_from_rx) {
  Payload tx;
  tx.ms = ms_from_rx;
  tx.seq = seq;
  tx.setpointF = setpointF;
  tx.flags = 0x01;  // ACK flag
  esp_err_t e = esp_now_send(COMM_UI_MAC, (uint8_t*) &tx, sizeof(tx));
  Serial.printf("CTRL TX ACK -> %s seq=%u err=%d\n", macStr(COMM_UI_MAC).c_str(), seq, (int) e);
}
...
void onRecv(const esp_now_recv_info_t* info, const uint8_t* data, int data_len) {
  ...
  // echo ACK
  Payload tx{rx.ms, rx.seq, rx.setpointF, 0x01};
  esp_err_t e = esp_now_send(COMM_UI_MAC, (uint8_t*) &tx, sizeof(tx));
  Serial.printf("CTRL TX ACK -> %s seq=%u err=%d\n", m, rx.seq, (int) e);
}
